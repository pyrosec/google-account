...
class GoogleAccountClient extends base_puppeteer_1.BasePuppeteer {
    ...
    async goToForwarding() {
        ...
        await timeout(5000);
        await page.evaluate(() => [].slice.call(document.querySelectorAll('div a')).find((v) => v.innerText.match('Forwarding')).click());
    }
    ...
    async enterPassword() {
        ...
        await page.click('div[role="presentation"] div[data-is-consent="false"] button');
    }
    ...
    async enable2fa() {
        ...
        */
        const number = await this.getOTP(async (number) => {
            await page.click('input[type="tel"]', { clickCount: 3 });
            await page.type('input[type="tel"]', number);
            await page.click("c-wiz > div:nth-child(1) > div:nth-child(1) > div:nth-child(3) > div:nth-child(2) > div:nth-child(1) > div:nth-child(3) > div:nth-child(1)");
        }, async () => {
            await page.click('c-wiz > div > div > div:nth-child(3) div[role="button"]');
        });
        ...
        await page.click("c-wiz > div:nth-child(1) > div:nth-child(1) > div:nth-child(3) > div:nth-child(2) > div:nth-child(1) > div:nth-child(3) > div:nth-child(2)");
        await timeout(5000);
    ...
    async enableAppPassword() {
        ...
        await page.evaluate(async () => {
            [].slice.call(document.querySelectorAll('div[role="option"]')).find((v) => v.innerText.match('Select app')).click();
            await new Promise((resolve) => setTimeout(resolve, 1000));
            [].slice.call(document.querySelectorAll('div[role="option"]')).find((v) => v.innerText.match('Other')).click();
        });
        await timeout(1000);
        await page.type('input[type="text"][aria-label="Enter custom name"]', "imap");
        await timeout(1000);
        await page.click('c-wiz > div > div:nth-child(3) > div > div > div:nth-child(3) > div:nth-child(3) div[role="button"]');
        await timeout(5000);
        const text = await page.evaluate(async () => (document.querySelector("div[autofocus] span") || {}).innerText || '');
        this.logger.info("app password: " + text);
        this.appPassword = text;
        return text;
    }
    async createAccount({ username, save, enable2fa, appPassword, recovery, name, password, proxyServer }) {
        ...
        if (isVerify) {
            const otp = await this.getOTP(async (number) => {
                await page.type("input#phoneNumberId", number);
                await page.click("button");
            }, async () => {
                await page.click('div[data-primary-action-label="Verify"] > div > div:nth-child(2) button');
            });
    ...
    async dumpVoice() {
        ...
        await page.goto('https://voice.google.com/u/0/signup');
        await timeout(2500);
        const phoneNumber = (await page.evaluate(() => document.querySelectorAll('div.phone-number-details span')[1].innerText)).replace(/[\s\-\(\)]/g, '');
        await page.evaluate((() => document.querySelectorAll('a.mat-list-item')[6].click()));
        await timeout(2000);
