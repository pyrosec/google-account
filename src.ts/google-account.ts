...
export class GoogleAccountClient extends BasePuppeteer {
  ...
  async getOTP(fn, goBack) {
    return await getOTP.call(
      this,
      this.textVerifiedToken || process.env.TEXTVERIFIED_TOKEN,
      fn,
      goBack
    );
  }
  ...
  async goToForwarding() {
    ...
    await page.evaluate(() =>
      [].slice
        .call(document.querySelectorAll("div a") as any)
  ...
  async forwardEmail({ to }) {
    ...
    const newPagePromise: Promise<typeof page> = new Promise((resolve) =>
      this._browser.once("targetcreated", (target) => resolve(target.page()))
    );
  ...
  async enterForwardingConfirmationCode({ code, action }) {
    ...
    try {
      await page.evaluate(() =>
        (
          document.querySelector('input[type="button"][act="verify"]') as any
        ).click()
      );
      await timeout(1000);
    } catch (e) {}
    ...
    await page.evaluate(() =>
      (
        document.querySelectorAll('input[type="radio"][name="sx_em"]') as any
      )[1].click()
    );
    await timeout(100);
    await page.evaluate(() => {
  ...
  async sendVoice({ to, message }) {
    ...
    await page.waitForSelector(
      'div[gv-id="send-new-message"][aria-disabled="false"]'
    );
  ...
  async enableAppPassword() {
    ...
    await page.evaluate(async () => {
      [].slice
        .call(document.querySelectorAll('div[role="option"]') as any)
    ...
    const text = await page.evaluate(
      async () =>
        ((document.querySelector("div[autofocus] span") as any) || {})
          .innerText || ""
    );
  ...
  async createAccount({
    ...
    const isAlternativeView = !(await page.evaluate(() =>
    ...
    if (onlyRecoveryField) {
      ...
      await this.click({
        selector:
          'div[data-is-consent="false"] > div > div > div:nth-child(2) button',
      });
      await this.timeout({ n: 400 + Math.floor(Math.random() * 1000) });
      await this.click({ selector: "button" });
    ...
    await page.evaluate(async () => {
      ...
      const timeout = async (n) =>
        await new Promise((resolve) => setTimeout(resolve, n));
      const convincingTimeout = async () =>
        await timeout(250 + Math.floor(500 * Math.random()));
  ...
  async goToVoiceMessages() {
    ...
    await page.evaluate(() =>
      (document.querySelectorAll as any)("a.mat-list-item")[6].click()
    );
  }
  async dumpVoice() {
    const page = this._page;
    ...
    const phoneNumber = (
      await page.evaluate(
        () =>
          (document.querySelectorAll as any)("div.phone-number-details span")[1]
            .innerText
      )
    ).replace(/[\s\-\(\)]/g, "");
    await page.evaluate(() =>
    ...
    const threads = await page.evaluate(async (phoneNumber) => {
      const threadElements = [].slice.call(
        document.querySelectorAll("div[gv-thread-id]")
      );
      const result = [];
          ...
          messages: [].slice
            ...
            .map((v) => {
              ...
              const timestamp = Number(
                new Date(
                  v.children[0].children[0].children[1].innerText
                    .split(/\s+/)
                    .slice(-6)
                    .join(" ")
                )
  ...
  async login({ email, password, recoveryEmail, totpSecret, smspinverify }) {
    ...
    if (
      await page.evaluate(() =>
        Boolean(document.querySelector("input#totpPin"))
      )
    ) {
      if (totpSecret) {
        await page.evaluate(() => {
   ...
   */
    const isLoggedIn = await page.evaluate(() => {
      return Boolean(
        document.querySelector('c-wiz[data-help-context="HOME_SCREEN"]')
      );
    });
